...
export default function Performance({ data }) {
    ...
    const [dataSet, setDataSet] = useState(data.slice(currentIndex, 3))
    ...
    const gotoNext = () => {
        (((currentIndex + 1) * 3) < data?.length) && setCurrentIndex(currentIndex + 1)
    }
    ...
    return <Box sx={{ width: '100%', height: '100%', }}>
        {data?.length ? <Box sx={{ width: '100%', height: '100%', }}>
            ...
            <Box sx={{
                ...
                {dataSet && <AverageRating title={user === 'all' ? 'All Performance' : `${dataSet?.find(i => i?.email === user)?.fullName}'s Performance`}
                    items={dataSet} color={dataSet?.find(i => i?.email === user)?.color || '#5BB5F6'}
                    total={4} grouped={true} full={user === 'all'} />}
            </Box>
            ...
            <Box sx={{ width: '100%', height: '100%', position: 'relative' }}>
                ...
                >
                    ...
                    {/* <Bar dataKey="supervisorRating" fill={supervisorColor} /> */}
                    <Bar dataKey='rating' >
                        {
                            dataSet.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={entry.color} /* width={entry?.blank ? 0 : 30} */ />
                            ))
                        }
                    </Bar>
            ...
            : <Typography sx={{ fontSize: 13, fontWeight: 600, textAlign: 'center', py: 2, px: 2 }}>
                No Handler Performance Evaluation
            </Typography>}
    </Box>
}