...
export default function ContactEmail({ clientId, fullName, emailAddress }) {
    ...
    return <Box sx={{
        ...
        borderRadius: '16px', width: '100%', height: '100%', display: 'flex', flexDirection: 'column'
    }}>
        {/* Heading */}
        <Box sx={{
        ...
        {open && <Box>
            ...
            <Box sx={{
                ...
                <Button variant="text"
                    ...
                    Sent ({emails?.sentMessages?.length || 0/* filter(i => i?.status === 'sent')?.length || 0 */})
                </Button>
            ...
            {emails ? <Box sx={{
                ...
                {emails[currentTab === 'sent' ? 'sentMessages' : 'receivedMessages'].map((email, index) => {
                    return <EmailSummaryCard key={index} email={email} />
                })}
            </Box> : <Loader />}
        </Box>}
    </Box>
