...
export default function NoteDetails({ isReply, note, open, handleClose, title }) {
    const [initialValues, setInitialValues] = useState({
        message: '', email: note?.email, title: note?.title
    })
    ...
    return <SideModal open={open} handleClose={handleClose} title={title} >
        <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', maxWidth: '100%' }}>
                ...
                {isReply
                    ? <Box sx={{ px: 3 }}>
                        <ShowMoreText value={note?.details} sx={{ fontSize: 12, py: 1 }} />
                    </Box>
            ...
            {/* Attachments */}
            {Boolean(note?.attachments?.length) && <Box sx={{ display: 'flex', my: 2, flexDirection: 'column', maxWidth: '100%' }}>
