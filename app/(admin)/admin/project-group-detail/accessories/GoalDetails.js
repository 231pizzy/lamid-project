...
import { changeGoalStatus, changeTaskStatus, getEmailAbbreviation, getProjectChats, sendFileToChat, sendGroupChat } from "../helper";
...
export function GoalDetails({ goal, setShowGoalDetails, projectId, selectedWorkphaseIndex, selectedGoalName }) {
    ...
    useEffect(() => {
        if (state.files) {
            ...
            sendFileToChat({
                projectId: projectId, fileArray: state.files,
    ...
    const handleChangeGoalStatus = async () => {
        try {
            ...
            changeGoalStatus({
                projectId: projectId, phaseIndex: selectedWorkphaseIndex, goalName: selectedGoalName, newStatus: updatedStatus,
                dataProcessor: (result) => {
                    if (result)
    ...
    const handleSendChat = (event) => {
        if (state.message.length) {
            ...
            sendGroupChat({
                projectId: projectId, linkArray: linkArray, link: Boolean(linkArray), message: state.message,
                dataProcessor: (result) => {
                    if (result) {
                        updateState({
    ...
    const handleChangeTaskStatus = async (task) => {
        try {
            ...
            changeTaskStatus({
                projectId: projectId, phaseIndex: selectedWorkphaseIndex, goalName: selectedGoalName, taskIndex: selectedTaskIndex, newStatus: updatedStatus,
                dataProcessor: (result) => {
                    if (result)
    ...
    return (
        <Box sx={{ maxHeight: '82vh', overflowY: 'hidden' }}>
                                ...
                                return <Box key={index} sx={{
                                    ...
                                    <Box sx={{
                                        ...
                                        <Button variant="text" sx={{
                                            ...
                                            onClick={() => openTaskDetails(task, index)}
                                        >
            ...
            <Modal open={state.openGallery} onClose={closeGallery}>
                <div>
                    <Gallery closeView={closeGallery} projectId={projectId} />
                </div>
            </Modal>
        </Box>
    )
