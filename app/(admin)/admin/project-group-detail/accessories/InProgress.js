...
export default function InProgress({  projectData, setShowGoalDetails, setSelectedGoal, setSelectedGoalName }) {
    ...
    const InprogressGoalsCount = projectData.reduce((count, phase) => {
        return count + (phase.workPhases || []).flatMap(wp => wp.goals || []).filter(goal => goal.goalStatus === 'In progress').length;
    }, 0);
    console.log("Data in To do:", projectData)
    ...
    return (
        <div>
            <Box sx={{ maxWidth: { xs: '100%', md: '400px' }, minWidth: { xs: '100%', md: '400px' }, borderRadius: '16px', mt: 3, border: '2px dashed #1C1D221A' }}>
                {/* Heading */}
                <Box sx={{
                    px: 3, py: 2, bgcolor: '#F293231A', borderRadius: '16px 16px 0 0',
                    display: 'flex', alignItems: 'center'
                }}>
                    {/* status Icon */}
                    <div style={{ backgroundColor: '#F293231A', border: `1px solid #F29323`, borderRadius: '50%' }}>
                        <InProgressIcon style={{ color: '#F29323', fontSize: 34 }} />
                    </div>
                ...
                {projectData.map((phase, phaseIndex) => (
                        phase.workPhases && phase.workPhases.length > 0 && (
                            phase.workPhases.flatMap(wp => wp.goals || []).filter(goal => goal.goalStatus === 'In progress').map((goal, goalIndex) => {
                            ...
                            return (
                                <Box key={`${phaseIndex}-${goalIndex}`} style={{ backgroundColor: 'white' }} className="mb-6 rounded-lg cursor-pointer" onClick={() => handleTaskClick(goal, goal.goalName)}>
                                    ...
                                    {/* Time spent */}
                                    <Box sx={{ mx: 2, my: 1.5 }}>
                                        <Box sx={{ alignItems: 'center', justifyContent: 'space-between', color: "rgba(242, 147, 35, 1)", mb: 2 }}>
                                            <p sx={{ fontWeight: 600, fontSize: 14, mb: 1.5, }}>
                                                Time Spent:<span> {Math.floor(totalHours * 0.3)}hrs {Math.floor(totalMinutes * 0.3)}mins out of {totalHours}hrs {totalMinutes}mins.</span>
                                            </p>
                                        ...
                                        <div style={{ width: '100%', height: '4px', backgroundColor: '#D9D9D9', borderRadius: '2px', marginBottom: '10px', position: 'relative' }}>
                                            <div style={{ width: '30%', height: '100%', backgroundColor: 'rgba(242, 147, 35, 1)', borderRadius: '2px', transition: 'width 0.3s ease-in-out' }}></div>
                                        </div>
                                    </Box>
                                    ...
                                    <Box>
                                        ...
                                        <Box sx={{ mx: 2, my: 1.5, padding: 1, }}>
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                ...
                                                {/* eg. 0/4 */}
                                                <Typography sx={{ fontWeight: 600, fontSize: 14, mb: 1.5, color: 'rgba(51, 51, 51, 1)', }}>
                                                    2/5
                                                </Typography>
                                            </Box>
                                            ...
                                            {/* Progress bar */}
                                            <div style={{ width: '100%', height: '4px', backgroundColor: '#D9D9D9', borderRadius: '2px', marginBottom: '10px', position: 'relative' }}>
                                                <div style={{ width: '40%', height: '100%', backgroundColor: 'rgba(191, 6, 6, 1)', borderRadius: '2px', transition: 'width 0.3s ease-in-out' }}></div>
                                            </div>
