...
export default function Completed({ projectData, setShowGoalDetails, setSelectedGoal,setSelectedGoalName }) {
    ...
    const CompletedGoalsCount = projectData.reduce((count, phase) => {
            ...
            <Box sx={{ maxWidth: { xs: '100%', md: '400px' }, minWidth: { xs: '100%', md: '400px' }, borderRadius: '16px', mt: 3, border: '2px dashed #1C1D221A' }}>
                ...
                {projectData.map((phase, phaseIndex) => (
                        phase.workPhases && phase.workPhases.length > 0 && (
                            ...
                            return (
                                <Box key={`${phaseIndex}-${goalIndex}`} style={{ backgroundColor: 'white' }} className="mb-6 rounded-lg cursor-pointer" onClick={() => handleTaskClick(goal, goalIndex)}>
                                    {/* Goal name */}
                                    <Box>
