...
import HandlerPerformanceRenderer from "@/Components/Table/HandlerPerformanceRenderer";
import GoalsRenderer from "@/Components/Table/GoalsRenderer";
import ProjectRenderer from "@/Components/Table/ProjectRenderer";
...
export default function ClientTable({ }) {
    ...
    const NarrowHeader = ({ value }) => {
        return <Box sx={{
            ...
            alignItems: 'center', mx: 'auto', textTransform: 'uppercase'
        }}>
            <Typography
                //key={index}
                sx={{ fontSize: 11, whiteSpace: 'pre-wrap', textAlign: 'center', fontWeight: 600, }}>
                {value}
            </Typography>
        </Box>
    }
    ...
    const [columnDefs,] = useState([
        ...
        {
            field: 'handlerPerformance',
            minWidth: 150,
            cellRenderer: HandlerPerformanceRenderer,
            headerComponent: (props) => <NarrowHeader value={`Handler's Performance`} />,
            headerName: `Handler's Performance`,
        },
        {
            field: 'attachment',
            minWidth: 150,
        ...
        },
        {
            field: 'goals',
            minWidth: 100,
            cellRenderer: GoalsRenderer,
            headerComponent: (props) => <NarrowHeader value={'GOALS'} />,
            headerName: 'GOALS',
        },
        {
            field: 'projects',
            ...
            headerComponent: (props) => <NarrowHeader value={'Projects'} />,
            headerName: 'Projects',
        },
    ]);
    ...
    return <Box sx={{ height: '100%' }}>
        ...
        <Box sx={{ height: 'calc(100vh - 185px)', width: '100%' }}>
            {Boolean(data) && <Table headingArray={columnDefs}
                ...
                unpublishEndpoint={''}
                cancelEndpoint={''}
                valuesURL={`/api/get-contact?stage=client`}
