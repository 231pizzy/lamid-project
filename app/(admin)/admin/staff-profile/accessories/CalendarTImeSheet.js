...
export function CalendarTimeSheet(prop) {
    ...
    const [state, setState] = useState({
        ...
        endDate: prop?.endDate, usedTimeToday: '0',
        ...
        //   currentDateSet:{start:prop?.startDate, end:moment(prop?.startDate,'yyyy-MM-DD').add(5,'days').toDate()}
    });
            ...
            (Number(event.hours) === 0 && Number(event.minutes)) ?
                ...
                <Box sx={{
                    ...
                    <Box sx={{
                        ...
                        {/* State of the task */}
                        <Typography sx={{ color: 'black', p: .5, fontWeight: 600, fontSize: { xs: 11, md: 13 }, display: 'flex', borderRadius: '8px', alignItems: 'center', bgcolor: lighten(stateData.iconBgColor, 0.3) }}>
                            <Avatar sx={{
                                mr: 1,
                        ...
                        {/* Time */}
                        <Typography sx={{
                            bgcolor: 'rgba(255, 255, 255, 0.8)', fontWeight: 600,
                            fontSize: { xs: 11, md: 13 }, borderRadius: '10px', p: .5,
                            color: '#8D8D8D', display: 'flex', alignItems: 'center',
                        }}>
                            <TimerIcon sx={{ fontSize: 13, mr: .5 }} />
                            {moment(event.start).format('h:mm A')} - {moment(event.end).format('h:mm A')} ({formatUsedTime(Number(event.minutes))})
                        </Typography>
                    </Box>
    ...
    const gotoDate = (date) => {
        ...
        prop.gotoDate(date)
        //   updateState({ usedTimeToday: getUsedTimeToday(date) })
    }
            ...
            <Box sx={{ pb: .5 }}>
                ...
                <Box sx={{
                    ...
                    {Array.from({ length: 5 }, (v, index) => {
                        ...
                        const isSelectedDate = moment(date).isSame(moment(state.date), 'day'); //check if it's the selected date
