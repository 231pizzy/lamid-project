...
export default function Todo({ teamId, teamGoals, setShowGoalDetails, setSelectedGoal }) {
            ...
            <Box sx={{ maxWidth: { xs: '100%', md: '400px' }, minWidth: { xs: '100%', md: '400px' }, borderRadius: '16px', mt: 3, border: '2px dashed #1C1D221A' }}>
                ...
                <Box sx={{
                    ...
                    <Typography sx={{ fontWeight: 700, fontSize: 16, ml: 4, textTransform: 'uppercase' }}>
                        To Do ({teamGoals.filter(goal => goal.goalStatus === 'To do').length})
                    </Typography>
                    ...
                    {creatingGoal ? (
                        <GoalForm teamId={teamId} handleCloseGoal={confirmCancelGoalCreation} />
                    ) : (
                        <></>
                    )}
                ...
                <Box sx={{ maxHeight: '40vh', overflowY: 'auto', bgcolor: 'rgba(28, 29, 34, 0.08)', p: 2 }}>
                    {teamGoals
                        .filter(goal => goal.goalStatus === 'To do')
                        .map((goal, index) => {
                                ...
                                <Box key={index} style={{ backgroundColor: 'white' }} className="mb-6 rounded-lg cursor-pointer" onClick={() => handleTaskClick(goal)}>
                                    ...
                                    <Box sx={{ mx: 2, my: 1.5 }}>
                                        {/* <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <p sx={{ fontWeight: 600, fontSize: 14, mb: 1.5, color: 'rgba(25, 211, 252, 1)' }}>
                                                Time Spent: <span> {totalcompletedHours}hr {totalCompletedMinutes}mins out of {totalHours}hr {totalMinutes}min</span>
                                            </p>
                                        ...
                                        <div style={{ width: '100%', height: '4px', backgroundColor: '#D9D9D9', borderRadius: '2px', marginBottom: '10px', position: 'relative' }}>
                                            <div style={{ width: '1%', height: '100%', backgroundColor: 'rgba(25, 211, 252, 1)', borderRadius: '2px', transition: 'width 0.3s ease-in-out' }}></div>
                                        </div>
                                    </Box>
                                    ...
                                    <Box>
                                        ...
                                        <Box sx={{ mx: 2, my: 1.5, padding: 1 }}>
                                            ...
                                            {/* Progress bar */}
                                            <div style={{ width: '100%', height: '4px', backgroundColor: '#D9D9D9', borderRadius: '2px', marginBottom: '10px', position: 'relative' }}>
                                                <div style={{ width: '1%', height: '100%', backgroundColor: 'rgba(191, 6, 6, 1)', borderRadius: '2px', transition: 'width 0.3s ease-in-out' }}></div>
                                            </div>
